...
/**
 * Approve Leave Command Handler
 *
 * Approves a pending leave request
 */
export const approveLeaveHandler: CommandHandler<ApproveLeavePayload> = {
  commandType: 'approve-leave',

  async handle(command: Command<ApproveLeavePayload>): Promise<CommandResult> {
    logger.info('Handling approve-leave command', {
    ...
    const event = await eventStore.appendEvent({
      ...
      event_data: {
        leave_request_id: command.payload.leave_request_id,
        approved_by: userId,
        approver_notes: command.payload.approver_notes || null,
...
/**
 * Validate approve leave payload
 */
function validatePayload(payload: ApproveLeavePayload) {
  const errors = [];
